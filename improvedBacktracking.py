...
def mathCheck(solution, rule, piece):
    ...
    big = list(piece.coords.values())[0]
    small = list(piece.coords.values())[1]
    if big < small:
        big = list(piece.coords.values())[1]
        small = list(piece.coords.values())[0]
...
def findMRV(board, coordinate, solution, values):
    ...
    for num in values:
        ...
        if i == len(board) or j == len(board):
            ...
            mrv[num] = len(findRemainingValues(soln, (i, j), list(range(1, len(board) + 1))))
...
def backtrack1(board, coordinate, pieces, solution, rules):
    ...
    valuesToTry = findRemainingValues(solution, coordinate, list(range(1, len(board) + 1)))
    if not valuesToTry:
        solution[i][j] = 0
    ...
    for num in mrv:
        ...
        global actualAssignment
        actualAssignment = actualAssignment + 1
...
print("Using Improved Backtracking(BT+MRV) Algorithm to solve the puzzle")
print("Time required:", endTime-startTime)
print("Number of assignment: ", actualAssignment)

print ("Improved Backtracking(BT+MRV): Generated with {} recursive calls.".format(count))
# print("Number of Assignment: ", assignment)
# print("Number of actualAssignment: ", actualAssignment)
