...
def geneticSearch(population, board, rules, pieces):
    ...
    while globalBest.fitness != 1:
        if previousBest.fitness < globalBest.fitness:
            previousBest = globalBest
        endTime = time.process_time()
        timeRequired = endTime-startTime
        # print("Time required test: ", timeRequired)
        if( timeRequired > 300):
            print("Time required: ", timeRequired)
            return previousBest, iterations
        # print("itteration: " + str(iterations) + " globalBest.fitness :" + str(previousBest.fitness))
        population = generatePopulation([], board, rules, pieces)
        length = len(population)
        globalBest = population[length - 1]
        localMaximumFitness = 0
        ...
        # while 1:
            ...
            while len(newPopulation) <= length-3:
                ...
                if solution1.fitness < solution2.fitness or np.random.rand(1)[0] > 0.90:
...
def readFile():
    ...
    # print(fieName)
    fileName = sys.argv[1]
    f = list(map(str.split, open(fileName)))
...
def backtrack0(coordinate, solution):
    ...
    valuesToTry = findRemainingValues(solution, coordinate, list(range(1, len(grid) + 1)))
...
def adjustingSwap(solution, board, rules):
    ...
    if len(unsolved) > len(solved):
        ...
        swapFrom = list(piece.coords.keys())[random]  # randomly picks an unsolved piece
        ...
        for r in range(len(grid)):
            if not (i, r) in piece.coords.keys() and \
                    not r == j and \
                    grid[i][r] in findPossibleValues(rules[piece.letter], list(range(1, len(grid)+1))):
        ...
        if swapj == j:
            ...
            for r in range(len(grid)):
                if not (r, j) in piece.coords.keys() and \
                        not r == i and \
                        grid[r][j] in findPossibleValues(rules[piece.letter], list(range(1, len(grid)+1))):
                    swapi = r
                    justSwappedInRow = 0
                    break
    else:
        ...
        piece = solved[0]
        swapFrom = list(piece.coords.keys())[np.random.randint(0, len(piece.coords.keys()))]
        i, j = swapFrom[0], swapFrom[1]
        swapi, swapj = i, j
...
msg = "No solution was found, this was the best solution generated."
if not fitness == 1.0:
    print(msg.format(y))
